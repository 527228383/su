			...
			<div class="Hui-article">
				<div class="cl pd-20">
					...
					<form action="act/dealWithClaimDetails"  method="post" enctype="multipart/form-data">
						<input type="hidden" name="b_insured_amount" value="${claimDetailsDto.b_insured_amount }"/>
						<input type="hidden" name="id" value="${claimDetailsDto.id }"/>
						<input type="hidden" name="processId" value="${claimDetailsDto.processId }"/>
						<!-- 隐藏域传递保单ID -->
						<input type="hidden" name="pol_id" value="${claimDetailsDto.id}"/>
						<table class="table table-border table-bordered table-hover table-bg">
							...
							<tr align="left">
								<td>事故发生时间:</td>
								<td><fmt:formatDate value="${claimDetailsDto.accident_date}" pattern="yyyy-MM-dd"/></td>
							</tr>
							<tr align="left">
							...
							<tr align="left">
								<td>结论:</td><td><input type="radio" name="mgr_dicision" value="1"/>通过
								<input type="radio" name="mgr_dicision" value="0"/>驳回</td>
							</tr>
						</table>
						<center>
