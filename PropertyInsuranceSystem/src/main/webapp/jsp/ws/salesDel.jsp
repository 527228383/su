...
<title></title>
	...
	<body >
		...
		<div class="breadcrumb"> 首页 <span class="c-gray en">&gt;</span> 客户理赔管理 <span class="c-gray en">&gt;</span> <span id="cdType"></span> </div>
		<div class="Hui-article">
			<div class="cl pd-20">
			<div id="searchDiv" style="border: 1px soild red">	
				<form action="/mannager/listSalesDel" method="get" id="page">
					<input type="hidden" name="currentPage" id="currentPage"/>
					<input type="hidden" name="emp_id" value="${claims[0].sales_id }" />
					...
					保险期限: <input type="text" value='<fmt:formatDate value="${pageUtil.query.startDate }" pattern="yyyy-MM-dd"/>' name="startDate" name="startDate" onclick="WdatePicker({readOnly:true,maxDate:'%y-%M-%d'})" readonly="readonly" class="Wdate" size="12"/>-<input type="text" onclick="WdatePicker()" readonly="readonly" class="Wdate" size="12" name="endDate" name="endDate" value='<fmt:formatDate value="${pageUtil.query.endDate }" pattern="yyyy-MM-dd"/>'/>
				...
				<table class="table table-border table-bordered table-hover table-bg">
					<thead>
						<tr>
							<th scope="col" colspan="9">业务员处理信息</th>
						</tr>
						<tr class="text-c">
							<th width="40">保单号</th>
							<th width="60">保险名称</th>
					...
					<c:forEach items="${claims}" var="p">
						<tr  class="tr2" onmouseover="this.style.backgroundColor='#ffff66';" onmouseout="this.style.backgroundColor='#d4e3e5';">
								...
								<td><font size="2.5px">${p.sales_name }</font></td>
								<td>
