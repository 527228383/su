...
<html>
  ...
  <body>
	<div class="Hui-article-box">
		...
		<c:if test="${listPolicys[0].b_state_id!=3}">
			待处理保单
		</c:if>
		<c:if test="${listPolicys[0].b_state_id==3}">
			已处理
 ...
 <span class="c-gray en">&gt;</span> <span id="cdType">
		<c:if test="${listPolicys[0].b_state_id==4}">
			待续报
		</c:if>
		<c:if test="${listPolicys[0].b_state_id==3}">
			生效中
		...
		<div class="mt-10">
			...
			<tbody>
			<c:forEach items="${listPolicys}" var="policyDto" varStatus="i">
				<tr align="center">
					<td>${policyDto.id}</td>
					<td>${policyDto.assurerDto.ass_name}</td>
					<td>${policyDto.assurerDto.ass_id_card}</td>
					<td>${policyDto.assurerDto.ass_tel}</td>
					<td>${policyDto.productDto.pro_name}</td>
					<td><fmt:formatDate value="${policyDto.b_lose_date}" pattern="yyyy-MM-dd"/></td>
					<td>${policyDto.b_coverage}</td>
					<td>${policyDto.b_state}</td>
					<td>
						<a href="sales/getPolicy/view/${policyDto.id}">查看保单</a>
						<c:choose>
