...
<html>
  ...
  <body>
	<div class="Hui-article-box">
		<div class="breadcrumb"> 首页 <span class="c-gray en">&gt;</span> 已处理 <span class="c-gray en">&gt;</span> <span id="cdType">审核中</span> </div>
		<div class="Hui-article">
			<div class="cl pd-20">
			<div id="searchDiv" style="border: 1px soild red">
		<p>
		<form action="act/getHaveDealWithClaimDetails" method="post" id="page">
		...
		<div class="mt-10">
			...
			<c:forEach items="${list}" var="policyDto" varStatus="i">
				<tr align="center">
					...
					<td>
						<c:if test="${policyDto.sales_dicision==1}">通过</c:if>
						<c:if test="${policyDto.sales_dicision==0}">驳回</c:if>
					</td>
					<td>
						<a href="sales/getPolicy/view/${policyDto.pol_id}">查看保单</a>
						<c:choose>
							<c:when test="${policyDto.b_state_id==4}">
								<a href="#">短信提醒</a>
							</c:when>
							<c:when test="${policyDto.b_state_id==5}">
								<a href="emp/getClaimDetailsDto/${policyDto.id}">理赔审核</a>
							</c:when>
							<c:otherwise>
								<a href="sales/claimDetail/${policyDto.id}">理赔进度</a>
							</c:otherwise>
						</c:choose>
					</td>
				</tr>
