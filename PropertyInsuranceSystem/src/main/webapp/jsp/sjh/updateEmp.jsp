	...
	<script>
		$(function(){
			...
			$("#upBtn").click(function(){
				...
				var $position = $("select[name='emp_position_id'] option:selected").val();
				var $province = $("select[name='emp_province_id'] option:selected").val();
				var $city = $("select[name='emp_city_id'] option:selected").val();
				alert($city);
				$.ajax({  
			        type:"post",  
			        url:"emp/updateEmployee",  
			        data:{"emp_position_id": $position, "id": $id, "emp_address": $address, "emp_hiredate": $joinTime, "emp_province_id": $province, "emp_city_id": $city},  
			        dataType:"text",  
			        error: function(data){alert("修改失败");},  
			        success:function(data){
			        	alert("修改成功!");
			        	window.location.href="emp/getEmpList";
			        }  
			    });
			});
