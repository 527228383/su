...
<title></title>
	...
	<body >
		...
		<div class="breadcrumb"> 首页 <span class="c-gray en">&gt;</span> 客户理赔管理 <span class="c-gray en">&gt;</span> <span id="cdType">所有</span> </div>
		<div class="Hui-article">
			<div class="cl pd-20">
			<div id="searchDiv" style="border: 1px soild red">
				<form action="act/getAllDealWithClaimDetails" method="post" id="searchForm">
					...
					保险期限: <input type="text" value="${requestScope.pageUtil.query.startDate }" name="startDate" name="startDate" onclick="WdatePicker({readOnly:true,maxDate:'%y-%M-%d'})" readonly="readonly" class="Wdate" size="12"/>-<input type="text" onclick="WdatePicker()" readonly="readonly" class="Wdate" size="12" name="endDate" name="endDate" value="${requestScope.pageUtil.query.endDate }"/>
					...
					<tbody>
						<c:forEach items="${requestScope.list }" var="cd">
							<tr>
								...
								<td>${cd.b_coverage }</td>
								<td>
									<a href="sales/getPolicy/${cd.pol_id }">保单详情</a>
						...
						<c:if test="${requestScope.list == null }">
							<tr>
								<td colspan="8">
									<font color="red" size="3">暂时没有理赔列表</font>
								</td>
							</tr>
						</c:if>
					</tbody>
