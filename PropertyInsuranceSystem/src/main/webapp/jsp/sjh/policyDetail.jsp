...
<body>
	<div class="boxed_wrapper">
		 ...
		 <div class="container container-small well">
			...
			<br />
			<h4 align="center">知领保险股份有限公司保险单</h4>
			<br />
			<div class="float_right">
				<label class="input-group" style="margin-right: 10px;">
					保单号：<span>${policyDto.id}</span>
				</label>
			</div>
			<br /><br />
			<div>
			...
			<div class="container container-ss" style="width: 90%;">
				<div class="col-xs-6" style="background-color: white;">
					<br/>
					<label>
						投保人姓名：<span>${policyDto.insureDto.ins_name }</span>
					</label><br/><br/>
					<label>
						证件类型：<span>二代身份证</span>
					</label><br/><br/>
					<label>
						出生日期：<span>${fn:substring(policyDto.insureDto.ins_id_card,6,10)}-${fn:substring(policyDto.insureDto.ins_id_card,10,12)}-${fn:substring(policyDto.insureDto.ins_id_card,12,14)} </span>
					</label><br/><br/>
				</div>
				<div class="col-xs-6" style="background-color: white;">
					<br/>
					<label>
						<c:if test="${policyDto.insureDto.ins_gender=='1'}">
							性别：<span>男</span>
						</c:if>
						<c:if test="${policyDto.insureDto.ins_gender=='0'}">
							性别：<span>女</span>
						</c:if>
					</label><br/><br/>
					<label>
						证件号码：<span>${policyDto.insureDto.ins_id_card}</span>
					</label><br/><br/>
					<label>
						手机号码：<span>${policyDto.insureDto.ins_tel }</span>
					</label><br/><br/>
				</div>
			</div>
			...
			<div class="container container-ss" style="width: 90%;">
				<div class="col-xs-6" style="background-color: white;">
					<br/>
					<label>
						被保险人姓名：<span>${policyDto.assurerDto.ass_name }</span>
					</label><br/><br/>
					<label>
						<c:if test="${policyDto.assurerDto.ass_gender=='1'}">
							性别：<span>男</span>
						</c:if>
						<c:if test="${policyDto.assurerDto.ass_gender=='0'}">
							性别：<span>女</span>
						</c:if>
					</label><br/><br/>
					<label>
						证件号码：<span>${policyDto.assurerDto.ass_id_card }</span>
					</label><br/><br/>
					<label>
						手机号码：<span>${policyDto.assurerDto.ass_tel}</span>
					</label><br/><br/>
				</div>
				<div class="col-xs-6" style="background-color: white;">
					<br/>
					...
					<label>
						出生日期：<span>${fn:substring(policyDto.assurerDto.ass_id_card,6,10)}-${fn:substring(policyDto.assurerDto.ass_id_card,10,12)}-${fn:substring(policyDto.assurerDto.ass_id_card,12,14)} </span>
					</label><br/><br/>
					<label>
						&nbsp;
					</label><br/><br/>
			...
			<div class="container container-ss" style="width: 90%;">
				<div class="col-xs-6" style="background-color: white;">
					<br />
					<label >
						<span>${policyDto.productDto.pro_name}&nbsp;</span>损失保险
					</label>
				</div>
				<div class="col-xs-6" style="background-color: white;">
					<br />
					<label >
					<span>最高可赔付&nbsp;${policyDto.b_insured_amount}&nbsp;万元</span>
					</label>
				</div>	
			</div>
			<br /><br />
			...
			<div class="container container-ss" style="width: 90%;">
				<div style="background-color: white;">
					<br />
					<label>
						保险期限：<span><fmt:formatDate value="${policyDto.b_effect_date}" pattern="yyyy-MM-dd"/></span>零时起&nbsp;至&nbsp;<span><fmt:formatDate value="${policyDto.b_lose_date}" pattern="yyyy-MM-dd"/></span>二十四时止
					</label><br/><br/>
					<label>
						合计保费：<span style="font-size: 20px;color: orange;">${policyDto.b_coverage}</span>元
					</label><br/><br/>
				</div>
			...
			<div class="container container-ss" style="width: 90%;">
				<div style="background-color: white;">
					<br />
					<label>
						电子保单：发送至投保人的电子邮箱&nbsp;<span>${policyDto.insureDto.ins_email}</span>
					</label><br/><br/>
				</div>
