...
<body>
	...
	<br /><br />
		...
		<form action="<%=basePath%>insure/selectlipei" id="fm">
			<div class="col-xs-2" ">
				<!-- 时间筛选 -->
			...
			<table class="table table-hover">
				...
				<c:forEach items="${ls}" var="n">
					<tr>
						...
						<td>
							<p><a href="<%=basePath%>insure/selectPolicyByid/${n.id}">保单详情</a></p>
						...
						<c:choose>
							<c:when test="${n.b_state_id==5}">
								<td>待核实</td>
							</c:when>
							<c:when test="${n.b_state_id==8}">
								<td>已赔付</td>
							</c:when>
							<c:when test="${n.b_state_id==9}">
								<td>已驳回</td>
							</c:when>
							<c:otherwise>
								<td>核实中</td>
